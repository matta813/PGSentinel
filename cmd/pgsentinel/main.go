...
func main() {
	...
	defer stop()
	manager := collector.NewManager(store, log, collector.Schedule{Fast: cfg.FastInterval, Standard: cfg.StatsInterval, Slow: cfg.SlowInterval, Metadata: cfg.MetaInterval})
	go manager.Run(ctx)
