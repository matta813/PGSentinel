...
func TestQueryRegressionIncludesChangesInsideRegressionWindow(t *testing.T) {
	...
	if len(got) != 1 { t.Fatalf("findings=%#v", got) }
	evidence := ""
	for _, item := range got[0].Evidence { evidence += item.Label + ":" + item.Value + "\n" }
	if !strings.Contains(evidence, "Correlated change:deployment") || !strings.Contains(evidence, "release checkout 2.4") || strings.Contains(evidence, "too early") { t.Fatalf("evidence=%s", evidence) }
}
