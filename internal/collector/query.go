...
const queryStatsMetadataSQL = `SELECT stats_reset,pg_postmaster_start_time() FROM pg_stat_statements_info`
const queryStatsSQL = `SELECT queryid::text,d.datname,query,calls,total_exec_time,mean_exec_time,min_exec_time,max_exec_time,rows,shared_blks_hit,shared_blks_read,temp_blks_read,temp_blks_written,COALESCE(wal_bytes,0) FROM pg_stat_statements p JOIN pg_database d ON d.oid=p.dbid ORDER BY total_exec_time DESC LIMIT 200`
...
func (c *Core) CollectQueries(ctx context.Context) ([]models.QueryStat, bool, *time.Time, *time.Time, error) {
	var available bool
	if err := c.db.QueryRow(ctx, `SELECT EXISTS(SELECT 1 FROM pg_extension WHERE extname='pg_stat_statements')`).Scan(&available); err != nil || !available {
		return nil, available, nil, nil, err
	}
	var statsReset, postmasterStart *time.Time
	// Metadata is best effort: losing it disables comparisons across an uncertain boundary,
	// but must not make query visibility itself unavailable.
	_ = c.db.QueryRow(ctx, queryStatsMetadataSQL).Scan(&statsReset, &postmasterStart)
	rows, err := c.db.Query(ctx, queryStatsSQL)
	if err != nil {
		return nil, true, statsReset, postmasterStart, err
	}
	defer rows.Close()
	...
	return out, true, statsReset, postmasterStart, rows.Err()
}
