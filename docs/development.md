...
Collector failures are logged with server, database when applicable, resource kind, collector step, failure/retry counts, duration, and the underlying error. The first failure and any changed error are logged immediately; unchanged failures are summarized no more often than `PGSENTINEL_COLLECTOR_DIAGNOSTIC_INTERVAL` (default `5m`, accepted range `10s` to `1h`). Successful retries emit a recovery event, and restoring a snapshot emits `cached=true`. Lower values increase log volume without changing collection frequency.
