...
Metric maintenance runs hourly in one SQLite transaction. Raw samples age into 15-minute and 6-hour aggregates containing average, minimum, maximum, and sample count before the raw rows are deleted. Independent bounded retention windows delete expired aggregate tiers. History reads combine raw data with the finest retained aggregate and remain bounded by the API limit; the application never requires an external time-series service.

Operational API routes require a user-bound session. A compact server-side permission matrix gives administrators configuration and user management, operators read access plus finding acknowledgement, and viewers read-only operational access. A new data volume bootstraps the built-in `admin` account from `PGSENTINEL_ADMIN_PASSWORD` and restricts its first session to replacing that password. Salted Argon2id credentials and constrained roles are persisted in SQLite; the environment value never overwrites an existing account. Random session tokens are delivered in HttpOnly, SameSite=Strict cookies and stored only as hashes in process memory; restarts and role changes intentionally invalidate affected sessions. Login attempts are rate-limited with bounded state, unknown usernames take the same expensive verification path, and forwarded client addresses are accepted only from explicitly trusted proxy CIDRs.

Successful security and configuration mutations append a fixed-summary audit event. Login failures are also recorded without the attempted username or source address. Audit records never copy request bodies or secret configuration, are exposed through a bounded authenticated API, and are retained for 365 days with a 100,000-row hard cap.
...
Collectors are read-only and set `application_name=pgsentinel`. The scheduler independently runs fast (connections, locks, and wait events), standard (database statistics, queries, replication and WAL), slow (tables, indexes and vacuum), and metadata (configuration) cycles. Connections use short timeouts and a pool capped at two. Wait events use one bounded, cluster-wide `pg_stat_activity` query with a 500-row limit and 2,000-byte query-text bound; they do not fan out by database. On slow cycles, table and index collection fans out per database: it connects to each collectible database (templates excluded, up to the configured fan-out limit, largest first) and aggregates the results into a single snapshot per server.

Each resource has a persistent collection-status record containing the last attempt, last successful collection, expected interval and consecutive failure count. API responses derive `stale` after two missed expected intervals. A database fan-out that succeeds only in part is explicitly `partial`; failures preserve the last complete snapshot and never reconcile findings as resolved. Safe summaries are stored instead of raw driver errors to avoid persisting connection details.

`GET /api/v1/servers/{id}/wait-events` returns only the latest retained snapshot. No synthetic or inferred history is returned. The UI computes current counts and percentages from those exact rows and distinguishes a fresh empty snapshot from stale, partial, or unavailable evidence.
...
After a complete analyzer cycle, incident correlation considers a bounded recent finding set for that server. A deterministic graph connects only findings that began within 15 minutes and share a resource or an explicit PostgreSQL subsystem relationship. Connected components persist as incident summaries; detail timelines are derived from the associated finding lifecycle timestamps. Partial collection never rebuilds incident state, and no relationship is presented as causation.
