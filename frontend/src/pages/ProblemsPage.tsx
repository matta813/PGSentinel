...
function FindingRow({ finding, defaultOpen, onStatus }: { finding: Finding; defaultOpen: boolean; onStatus: (id: string, status: 'active' | 'acknowledged') => Promise<void> }) {
  return <details id={`finding-${finding.id}`} open={defaultOpen || undefined} className={`finding severity-${finding.severity.toLowerCase()}`}>
    ...
    <div className="finding-detail"><section><h3>What is wrong</h3><p>{finding.summary}</p>{finding.cause && <p>{finding.cause}</p>}</section><section><h3>Why it matters</h3><p>{finding.impact}</p></section><section className="finding-evidence"><h3>Evidence</h3>{finding.evidenceQuality && finding.evidenceQuality.state !== 'fresh' && <div className={`evidence-quality ${finding.evidenceQuality.state}`}><strong>{finding.evidenceQuality.state} evidence</strong><span>{finding.evidenceQuality.errorSummary} Existing findings remain open until a complete collection can verify recovery.</span></div>}<div className="evidence-grid">{finding.evidence?.map(item => <div key={item.label}><span>{item.label}</span><strong>{item.value}{item.unit && <small> {item.unit}</small>}</strong></div>)}</div></section><section><h3>What to investigate</h3><ol>{finding.suggestions?.map((suggestion, index) => <li key={index}><strong>{suggestion.title}</strong>{suggestion.detail && <p>{suggestion.detail}</p>}</li>)}</ol></section><footer><span>Confidence <strong>{finding.confidence}</strong></span>{finding.status === 'active' && <button className="button secondary" onClick={() => void onStatus(finding.id, 'acknowledged')}><Check /> Acknowledge</button>}{finding.status === 'acknowledged' && <button className="button secondary" onClick={() => void onStatus(finding.id, 'active')}><RotateCcw /> Reopen</button>}</footer></div>
  </details>
}
