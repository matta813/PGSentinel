...
export function OverviewPage() {
  ...
  return <>
    ...
    {data.servers.length === 0 ? <Empty title="No servers yet" detail="Add your first PostgreSQL server to start monitoring." action={<Link className="button primary" to="/servers">Add server</Link>} /> : <div className="overview-servers">{data.servers.map(server => { const quality = worstQuality(data.freshness?.[server.id] ?? []); return <article className="overview-server" key={server.id}><span className="server-icon"><ServerIcon /></span><div><strong>{server.name}</strong><code>{server.host}:{server.port}</code></div><div className="server-version"><span>Evidence</span><strong className={`freshness-state ${quality}`}>{quality}</strong></div><StatusIndicator status={server.status} /></article> })}</div>}
  </>
}
