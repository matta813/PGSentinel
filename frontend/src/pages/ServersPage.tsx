...
export function ServersPage() {
  ...
  function closeForm() { setForm(initial); setEditingID(''); setOpen(false); }
  function edit(server: Server) {
    ...
    setEditingID(server.id); setOpen(true);
  }
  async function submit(e: FormEvent) {
    e.preventDefault(); setMessage('');
    try {
      if (editingID) await api.put(`/servers/${editingID}`, form); else await api.post('/servers', form);
      closeForm(); void reload();
    } catch (e) { setMessage(e instanceof APIError ? `${e.message}. ${e.detail}` : 'Unable to save server'); }
  }
  async function test(id: string) {
    setMessage('Testing connection…');
    try { const v = await api.post<{ version: string }>(`/servers/${id}/test`); setMessage(`Connected to PostgreSQL ${v.version}`); void reload(); }
    catch (e) { setMessage(e instanceof APIError ? `${e.message}. ${e.detail}` : 'Connection failed'); }
  }
  ...
  return <>
    ...
    {open && <form className="server-form" onSubmit={submit}>
      ...
      <label>Tags (comma separated)<input value={form.tags.join(', ')} onChange={e => setForm({ ...form, tags: e.target.value.split(',') })} placeholder="production, eu" /></label>
