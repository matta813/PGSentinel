...
export function App() {
  ...
  return <MonitoringProvider><Routes><Route element={<AppLayout username={authentication.username} role={authentication.role} onLogout={() => setAuthentication('anonymous')} />}><Route index element={<OverviewPage />} /><Route path="problems" element={<ProblemsPage />} /><Route path="incidents" element={<IncidentsPage />} /><Route path="servers" element={<ServersPage />} />{authentication.role === 'administrator' && <><Route path="audit" element={<AuditPage />} /><Route path="users" element={<UsersPage currentUser={authentication.username} />} /><Route path="settings" element={<SettingsPage />} /></>}<Route path=":resource" element={<ResourcePage />} /></Route></Routes></MonitoringProvider>
}
